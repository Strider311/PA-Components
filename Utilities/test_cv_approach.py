...
from ImageProcessingApproach.SimpleOpenCvMultiSpectral import ImageProcessingClassifier
import os
import cv2
import pandas as pd
import numpy as np
from ImageProcessingApproach.RgbImageProcessingApproach import RgbImageProcessingClassifier
...
class AccuracyTester():
    def __init__(self, labels_path: str, rgb_folder: str, processed_folder: str, imageClassifier, confusionMatrix: ConfusionMatrix):
    ...
    def process_image(self, image_name: str):
        ...
        for index, row in rows:
            ...
            # Works with rgb classifier and simple classifier

            healthy_area = self.image_processor.extract_healthy_from_subimage(
                processed_file_name, x_min=x_min, x_max=x_max, y_min=y_min, y_max=y_max)

            unhealthy_area = self.image_processor.extract_unhealthy_from_subimage(
                processed_file_name, x_min=x_min, x_max=x_max, y_min=y_min, y_max=y_max)
...
rgb_dir = "C:\\Users\\saif_\\Main\\Source\\PrecisionAgriculture\\Data\\Input\\RGB_Images\\Train_Images"
labels_dir = "C:\\Users\\saif_\\Main\\Source\\PrecisionAgriculture\\Data\\Input\\RGB_Images\\Train_Labels_CSV.csv"

numpy_dir = os.path.join(root_dir, "numpy")
ndvi_dir = os.path.join(root_dir, "images")

confusionMatrix = ConfusionMatrix()
multiSpectralClassifier = ImageProcessingClassifier()
rgbCvClassifier = RgbImageProcessingClassifier()

accuracy_tester = AccuracyTester(
    labels_path=labels_dir, imageClassifier=rgbCvClassifier, processed_folder=numpy_dir, rgb_folder=rgb_dir, confusionMatrix=confusionMatrix)
